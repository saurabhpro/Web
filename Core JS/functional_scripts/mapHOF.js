...
const filteredArray = (array, func) => {
  let resultArr = [];
  for (const val of array) func(val) ? resultArr.push(val) : undefined;
...
const mappedArray = (array, func) => {
  let resultArr = [];
  for (const val of array) resultArr.push(func(val));
...
var resultOfMap = mappedArray(resultOfFilter, (book) => {
